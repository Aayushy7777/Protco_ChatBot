...
<div class="kpi-card kpi-{k.get('color','blue')}">
  ...
  <div class="kpi-detail">{_esc(k.get('detail',''))}</div>
    ...
    # Render only the first two charts in the screenshot layout
    charts_for_row = (charts or [])[:2]
    chart_cards = []
    for c in charts_for_row:
        chart_cards.append(
            f"""
<div class="chart-card">
  <div class="chart-title">{_esc(c.get('title','Chart'))}</div>
  <div class="chart-insight">{_esc(c.get('insight',''))}</div>
  <div class="chart-wrap"><canvas id="{_esc(c.get('id','chart'))}"></canvas></div>
</div>""".strip()
        )
    charts_html = "\n".join(chart_cards)
...
<body>
  ...
  <script>
    ...
    window.addEventListener('DOMContentLoaded', () => {{
      ...
      (CHARTS_DATA || []).slice(0,2).forEach(c => initChart(c));
